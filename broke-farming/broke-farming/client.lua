...
print("Script:         esx_broke-farming\r\nGescripted von: Broke Gaming\r\nStatus:         gestarted")
        ...
        for k, v in pairs(Config.Farm.Type) do 
            ...
            if distance < 12 then 
                ...
                if IsControlJustPressed(0, 38) then -- [E]
                    ...
                    local item = v.Value 
                    local count = v.Count
                    TriggerServerEvent("broke_farm:giveItem", item, count)
                end 
            end
        end
    end
...
RMenu.Add('broke_menu', 'main', RageUI.CreateMenu("Items verkaufen", "~b~Verkaufe deine Items", menuPosition["x"], menuPosition["y"]))
RMenu.Add('broke_menu', 'verkauf', RageUI.CreateSubMenu(RMenu:Get('broke_menu', 'main'), "Verkaufen", "~b~Verkaufe Items", menuPosition["x"], menuPosition["y"]))

Citizen.CreateThread(function()
    while true do 
        Citizen.Wait(0)
        RageUI.IsVisible(RMenu:Get('broke_menu', 'main'), true, true, true, function()
            RageUI.ButtonWithStyle("Items Verkaufen", "~b~Verkaufe Items", {RightLabel = "→→→"}, true, function() end, RMenu:Get('broke_menu', 'verkauf'))
        end, function()
        end)
        RageUI.IsVisible(RMenu:Get("broke_menu", 'verkauf'), true, true, true, function()
            for k, v in pairs(Config.Shop.Items) do 
                RageUI.ButtonWithStyle(v.Label, "Item Verkaufen", {RightLabel = "Preis: ~g~"..v.Price.."$~w~ / ~g~Item"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        anim()
                        TriggerServerEvent('broke_sell:SellItem', ItemValue, ItemPrice, ItemLabel)
            ...
            if distance < 8 then 
                ...
                if distance < 3.0 then 
                    ...
                    if IsControlJustReleased(0, 38) then -- [E]
                        if distance < 3.0 then
                            RageUI.Visible(RMenu:Get('broke_menu', 'main'), not RageUI.Visible(RMenu:Get('broke_menu', 'main')))
                        end
                    end
                end
            end
